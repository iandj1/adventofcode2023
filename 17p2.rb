...
while !new_nodes.empty?
  ...
  new_nodes.each do |row, col, direction, distance|
    ...
    directions.each do |new_dir, delta_coord|
      ...
      next if direction != :none && (direction != new_dir && distance < 4) # min distance of 4 before turn
      if new_dir == direction
        new_distance = distance + 1
      else
        new_distance = 1
      end
      next if new_distance > 10 # can't go too far in straight line
...
min = Float::MAX
[:east, :south].product((4..10).to_a).each do |dir_pair|
